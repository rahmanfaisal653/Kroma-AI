...
import { providerChatTargets, providerHeaders, openAiCompatibleBody, nativeOllamaBody, nativeOllamaToOpenAI, canUseProvider } from '../ai/customProvider.js';
...
router.post('/chat/completions', requireGatewayKey, async (req, res) => {
  ...
  const provider = configured ? { id: configured.id, baseUrl: configured.baseUrl, apiKey: configured.apiKey, chatPath: configured.chatPath, modelsPath: configured.modelsPath, name: configured.name, enabled: configured.enabled, visibility: configured.visibility } : null;
