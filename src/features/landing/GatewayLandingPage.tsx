...
const providers = [
  { id: 'pc-hitam', status: 'on', models: 5, latency: '812ms' },
  { id: 'pc-putih', status: 'on', models: 122, latency: '640ms' },
  { id: 'token-router', status: 'error', models: 0, latency: 'timeout' },
];
    ...
    <div className="mx-auto max-w-7xl px-5 py-6 space-y-7">
      ...
      <section className="rounded border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <MiniRoute provider="pc-hitam" model="llama3:latest" use="local chat / support" />
          <MiniRoute provider="pc-putih" model="qwen2.5-coder" use="coding / long context" />
          <MiniRoute provider="token-router" model="nvidia/nemotron" use="partner fallback" />
        </div>
      </section>

      <section className="grid gap-4 md:grid-cols-4">
        <Metric label="requests / 24h" value="184,229" sub="across 6 providers" />
        <Metric label="healthy providers" value="4 / 6" sub="2 degraded" tone="emerald" />
      ...
      <section id="features" className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[[ShieldCheck, 'Guardrails', 'Internal/Partner visibility, disabled provider blocking, API-key scoped routing.'], [Clock3, 'Provider health', 'Provider response includes status, error reason, and model count.'], [Database, 'Optional RAG', 'Inject Chroma-backed context only when requested.']].map(([Icon, title, desc]: any) => <Card key={title}><Icon size={17} className="text-[var(--color-text-muted)]" /><h3 className="mt-3 font-medium">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--color-text-muted)]">{desc}</p></Card>)}
        </div>
      </section>

      <section id="dashboard" className="rounded border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface-alt)] px-4 py-3"><div><p className="text-sm font-medium">Developer Dashboard</p><p className="font-mono text-xs text-[var(--color-text-muted)]">/providers · /keys · /usage · /knowledge</p></div><span className="rounded border border-emerald-500/40 bg-emerald-500/10 px-2 py-1 text-xs text-emerald-600">gateway on</span></div>
        <div className="grid lg:grid-cols-[320px_1fr]">
          <div className="border-b border-[var(--color-border)] p-4 lg:border-b-0 lg:border-r"><p className="mb-3 text-xs uppercase tracking-wider text-[var(--color-text-muted)]">Active providers</p><div className="space-y-2">{providers.map(p => <div key={p.id} className="flex items-center justify-between rounded border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2"><div><p className="font-mono text-sm">{p.id}</p><p className="text-xs text-[var(--color-text-muted)]">{p.models} models · {p.latency}</p></div><span className={p.status === 'on' ? 'text-emerald-600' : 'text-red-500'}>{p.status}</span></div>)}</div></div>
      ...
      <section id="docs" className="grid gap-6 pb-10 lg:grid-cols-[220px_1fr_220px]"><Card><p className="mb-3 text-xs uppercase tracking-wider text-[var(--color-text-muted)]">API Reference</p><p>/v1</p><p>/v1/providers</p><p>/v1/chat/completions</p></Card><Card><h2 className="text-xl font-semibold">OpenAI-compatible chat</h2><p className="mt-2 text-sm leading-6 text-[var(--color-text-muted)]">Use provider-prefixed model ids returned from <code>/v1/providers</code>.</p><div className="mt-4"><CodeBlock code={curl} /></div></Card><Card><p className="mb-3 text-xs uppercase tracking-wider text-[var(--color-text-muted)]">On this page</p><p>Authentication</p><p>Provider sync</p><p>Chat request</p></Card></section>
    </div>
  </div>;
}
