...
type Form = { id: string; originalId?: string; name: string; baseUrl: string; secret: string; mode: 'create' | 'edit' };

const emptyForm: Form = { id: '', name: '', baseUrl: '', secret: '', mode: 'create' };
...
export default function ModelsPage() {
  ...
  const save = async () => {
    ...
    const payload = { id: form.id, name: form.name, baseUrl: form.baseUrl, apiKey: form.secret };
  ...
  const reset = async (p: ProviderStatus) => {
  ...
  return <div className="h-full overflow-y-auto"><div className="max-w-5xl mx-auto p-6 space-y-6 animate-fade-in">
    <div className="flex items-start justify-between gap-3">
      <div><h1 className="text-xl font-bold text-[var(--color-text)]">Providers</h1><p className="text-sm text-[var(--color-text-muted)] mt-0.5">Tambah provider OpenAI-compatible: cukup isi URL dan API key. Model dipakai dengan format prefix/model.</p></div>
    ...
    <div className="grid md:grid-cols-3 gap-4">
      {providers.map(p => <div key={p.id} className="kroma-card-hover rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 space-y-3 backdrop-blur-xl">
        <div className="flex items-center justify-between gap-2"><h2 className="font-semibold text-[var(--color-text)]">{p.name || p.id}</h2><span className={`text-xs px-2 py-1 rounded-full ${p.status === 'ok' ? 'bg-emerald-500/10 text-emerald-400' : p.status === 'not_configured' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400'}`}>{p.status || 'unknown'}</span></div>
        ...
        <p className="text-xs text-[var(--color-text-muted)]">{p.custom ? 'custom' : p.overridden ? 'custom config' : 'env default'} · key: {p.configured ? 'set' : 'empty'}</p>
        {p.error && <p className="text-xs text-red-400">{p.error}</p>}
        <div className="flex gap-2"><Button size="sm" variant="outline" icon={<Edit2 size={13} />} onClick={() => setForm({ id: p.id, originalId: p.id, name: p.name || p.id, baseUrl: p.baseUrl, secret: '', mode: 'edit' })}>Edit</Button><Button size="sm" variant="outline" icon={<Trash2 size={13} />} onClick={() => reset(p)}>Delete</Button></div>
      </div>)}
    </div>
  </div></div>;
}
