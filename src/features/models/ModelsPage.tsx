...
type Form = { id: string; name: string; baseUrl: string; secret: string; chatPath: string; modelsPath: string; chatFormat: string; enabled: boolean; visibility: Visibility[] };

const emptyForm: Form = { id: '', name: '', baseUrl: '', secret: '', chatPath: '/chat/completions', modelsPath: '/models', chatFormat: 'openai', enabled: true, visibility: ['internal'] };
...
export default function ModelsPage() {
  ...
  return <div className="h-full overflow-y-auto"><div className="max-w-6xl mx-auto p-6 space-y-6 animate-fade-in">
    ...
    {form && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/45 px-4 py-10 backdrop-blur-sm"><div className="w-full max-w-2xl rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 space-y-3 shadow-xl">
      ...
      <div>
        <p className="text-xs text-[var(--color-text-muted)] mb-1">Chat Format</p>
        <select value={form.chatFormat} onChange={e => setForm({ ...form, chatFormat: e.target.value })} className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)] focus:outline-none focus:border-[var(--color-primary)]">
          <option value="openai">OpenAI (messages)</option>
          <option value="ollama">Ollama (prompt)</option>
        </select>
      </div>
