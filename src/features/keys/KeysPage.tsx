...
const emptyForm = { name: '', owner_name: '', note: '' };
...
export default function KeysPage() {
  ...
  const createKey = async () => {
    ...
    setLoading(true);
    try {
      const data = await internalKeysApi.create({ name: form.name.trim(), owner_type: type, owner_name: (form.owner_name || form.name).trim(), note: form.note.trim(), allowed_models: ['*'] });
    ...
    <div className="divide-y divide-[var(--color-border)]">
      {keys.map(k => <div key={k.id} className="px-5 py-4 flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-xs font-mono text-[var(--color-text-muted)]">{k.key ? '••••••••••••••••••••••••' : k.key_prefix}</p>
