...
export const providerStatusApi = {
  ...
  create: (data: { id?: string; name: string; baseUrl: string; apiKey?: string; chatPath?: string; modelsPath?: string; enabled?: boolean; visibility?: ProviderVisibility[] }) => http.post('/api/provider-status', data).then(r => r.data),
  update: (id: string, data: { name?: string; baseUrl: string; apiKey?: string; chatPath?: string; modelsPath?: string; enabled?: boolean; visibility?: ProviderVisibility[] }) => http.put(`/api/provider-status/${id}`, data).then(r => r.data),
